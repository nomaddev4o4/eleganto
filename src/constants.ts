export const useCases = [
  {
    image: "./images/image1.webp",
    link: "https://x.com/jrgarciadev/status/1924469908005323175",
  },
  {
    image: "./images/image2.webp",
    link: "https://www.heroui.com/",
  },
  {
    image: "./images/image3.webp",
    link: "https://heroui-chat-five.vercel.app/",
  },
  {
    image: "./images/image10.webp",
    link: "https://x.com/inkko44/status/1924126088210366470",
  },
  {
    image: "./images/image5.webp",
    link: "https://x.com/hero_ui/status/1909407579320860774",
  },
  {
    image: "./images/image6.webp",
    link: "https://www.coderabbit.ai/",
  },
  {
    image: "./images/image7.webp",
    link: "https://www.mochii.ai/",
  },
  {
    image: "./images/image8.webp",
    link: "https://lowdefy.com/",
  },
  {
    image: "./images/image9.webp",
    link: "https://chartbrew.com/",
  },
  {
    image: "./images/image10.webp",
    link: "https://www.likn.co/en",
  },
  {
    image: "./images/image11.webp",
    link: "https://www.encur.me/",
  },
  {
    image: "./images/image12.webp",
    link: "https://heroui.chat/",
  },
  {
    image: "./images/image1.webp",
    link: "https://x.com/jrgarciadev/status/1924469908005323175",
  },
  {
    image: "./images/image2.webp",
    link: "https://www.heroui.com/",
  },
  {
    image: "./images/image3.webp",
    link: "https://heroui-chat-five.vercel.app/",
  },
  {
    image: "./images/image10.webp",
    link: "https://x.com/inkko44/status/1924126088210366470",
  },
  {
    image: "./images/image5.webp",
    link: "https://x.com/hero_ui/status/1909407579320860774",
  },
  {
    image: "./images/image6.webp",
    link: "https://www.coderabbit.ai/",
  },
  {
    image: "./images/image7.webp",
    link: "https://www.mochii.ai/",
  },
  {
    image: "./images/image8.webp",
    link: "https://lowdefy.com/",
  },
  {
    image: "./images/image9.webp",
    link: "https://chartbrew.com/",
  },
  {
    image: "./images/image10.webp",
    link: "https://www.likn.co/en",
  },
  {
    image: "./images/image11.webp",
    link: "https://www.encur.me/",
  },
  {
    image: "./images/image12.webp",
    link: "https://heroui.chat/",
  },
  {
    image: "./images/image1.webp",
    link: "https://x.com/jrgarciadev/status/1924469908005323175",
  },
  {
    image: "./images/image2.webp",
    link: "https://www.heroui.com/",
  },
  {
    image: "./images/image3.webp",
    link: "https://heroui-chat-five.vercel.app/",
  },
  {
    image: "./images/image10.webp",
    link: "https://x.com/inkko44/status/1924126088210366470",
  },
  {
    image: "./images/image5.webp",
    link: "https://x.com/hero_ui/status/1909407579320860774",
  },
  {
    image: "./images/image6.webp",
    link: "https://www.coderabbit.ai/",
  },
  {
    image: "./images/image7.webp",
    link: "https://www.mochii.ai/",
  },
  {
    image: "./images/image8.webp",
    link: "https://lowdefy.com/",
  },
  {
    image: "./images/image9.webp",
    link: "https://chartbrew.com/",
  },
  {
    image: "./images/image10.webp",
    link: "https://www.likn.co/en",
  },
  {
    image: "./images/image11.webp",
    link: "https://www.encur.me/",
  },
  {
    image: "./images/image12.webp",
    link: "https://heroui.chat/",
  },
];

export const features = [
  {
    title: "AI-Powered Generation",
    description:
      "Transform natural language prompts into production-ready React components instantly with AI that understands design patterns and accessibility standards.",
    src: "/videos/heroui-dev-mode.mp4",
    color: "linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)",
  },
  {
    title: "Smart Code Navigation",
    description:
      "Effortlessly explore and understand your generated components with intelligent file navigation and AI-assisted code comprehension.",
    src: "/videos/heroui-live-file-navigation.mp4",
    color: "linear-gradient(135deg, #0ea5e9 0%, #3b82f6 100%)",
  },
  {
    title: "Intelligent Batch Editing",
    description:
      "Make sweeping changes across multiple components simultaneously with AI that understands relationships and applies consistent modifications.",
    src: "/videos/edit-multiple-elements-at-once.mp4",
    color: "linear-gradient(135deg, #10b981 0%, #059669 100%)",
  },
  {
    title: "One-Click Deployment",
    description:
      "Deploy your AI-generated UIs instantly with custom domains and automatic optimization—from prompt to production in minutes.",
    src: "/videos/heroui-custom-domains.mp4",
    color: "linear-gradient(135deg, #f43f5e 0%, #e11d48 100%)",
  },
  {
    title: "HeroUI Integration",
    description:
      "Built on the powerful HeroUI component library, ensuring your generated interfaces are beautiful, accessible, and production-ready from day one.",
    src: "/videos/heroui.mp4",
    color: "linear-gradient(135deg, #8b5cf6 0%, #d946ef 100%)",
  },
];

export const pricingPlans = [
  {
    id: "starter",
    name: "Starter",
    pricing: {
      amount: 9,
      currency: "$",
      period: "month",
      display: "$9",
      periodDisplay: "/month",
    },
    description: "Perfect for individuals exploring AI-powered design",
    features: [
      "50 AI generations per month",
      "Basic component library",
      "Community support",
      "Export to React/HTML",
      "Standard templates",
    ],
    cta: {
      text: "Start Creating",
      variant: "outline",
    },
    popular: false,
    badge: null,
  },
  {
    id: "professional",
    name: "Professional",
    pricing: {
      amount: 29,
      currency: "$",
      period: "month",
      display: "$29",
      periodDisplay: "/month",
    },
    description: "Ideal for professionals and growing teams",
    features: [
      "Unlimited AI generations",
      "Premium component library",
      "Priority support",
      "Advanced customization",
      "Team collaboration",
      "Custom domains",
      "Version history",
    ],
    cta: {
      text: "Start Creating",
      variant: "gradient",
    },
    popular: true,
    badge: {
      text: "Most Popular",
      color: "from-purple-500/80 to-pink-500/80",
    },
  },
  {
    id: "enterprise",
    name: "Enterprise",
    pricing: {
      amount: 99,
      currency: "$",
      period: "month",
      display: "$99",
      periodDisplay: "/month",
    },
    description: "For teams building at scale with AI",
    features: [
      "Everything in Professional",
      "White-label solutions",
      "24/7 dedicated support",
      "Custom AI training",
      "Advanced integrations",
      "Dedicated account manager",
      "SLA guarantee",
    ],
    cta: {
      text: "Contact Sales",
      variant: "outline",
    },
    popular: false,
    badge: null,
  },
];

export const tweets = [
  "https://x.com/jrgarciadev/status/1924469908005323175",
  "https://x.com/DhruvalGolakiya/status/1924127336020009337",
  "https://x.com/inkko44/status/1924126088210366470",
  "https://x.com/jrgarciadev/status/1923489702704771265",
  "https://x.com/meetbagda035/status/1916377999861023072",
  "https://x.com/therahulogy/status/1913583737733783944",
  "https://x.com/benswerd/status/1912209694057472021",
  "https://x.com/codesandbox/status/1906779656050680249",
  "https://x.com/genzpatel/status/1906734880823853514",
  "https://x.com/stopachka/status/1906751668957618321",
];
